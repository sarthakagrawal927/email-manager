...
const bulkPatterns = [/\b(newsletter|digest|weekly|roundup|webinar|promotion|sale|unsubscribe)\b/i];
...
export function triageEmails(emails: Email[]): TriageQueue[] {
  const items = emails.map((email): TriageItem => {
    ...
    if (wantsResponse || urgent || starred) {
      ...
      queue = 'unsubscribe';
      reason = email.unsubscribeLink
        ? 'Unsubscribe link detected'
        : 'Looks like recurring bulk mail';
